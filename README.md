...
#### 1. 잘했던 점
- 처음에는 벽돌깨기처럼 단순한 벽돌처럼 구현했으나, 몬스터로 구현하고 내려올 때 애니메이션을 rotaion값을 줘서 구현해봤다.
- 그리고 그냥 깨는것만이 아닌 한 줄 지워주는 몬스터, 십자를 지워주는 몬스터, 랜덤으로 캐릭터를 지워주는 특수 몬스터를 구현했다.

#### 2. 아쉬웠던 점
- 몬스터들이 패들을 공격하는 부분이 있었으면 좀 더 재미가 더해지지 않았을까 하는 아쉬운 점이 있었다.
